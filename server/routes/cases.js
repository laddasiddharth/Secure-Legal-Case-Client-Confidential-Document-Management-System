...
router.post('/', authMiddleware, roleMiddleware('lawyer', 'admin'), async (req, res) => {
  try {
...
// @desc    Update case
// @access  Private (Lawyer, Admin only)
router.put('/:id', authMiddleware, roleMiddleware('lawyer', 'admin'), async (req, res) => {
  try {
    const caseData = await Case.findById(req.params.id);
...
router.get('/stats/summary', authMiddleware, async (req, res) => {
  try {
    ...
    const activeCases = await Case.countDocuments({ ...filter, status: 'open' });
    const closedCases = await Case.countDocuments({ ...filter, status: 'closed' });
    const pendingCases = await Case.countDocuments({ ...filter, status: 'pending' });
...
// @access  Private (Lawyer, Admin only)
router.get('/clients/list', authMiddleware, roleMiddleware('lawyer', 'admin'), async (req, res) => {
  try {
